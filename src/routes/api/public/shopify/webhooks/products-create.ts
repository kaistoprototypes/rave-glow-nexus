...
export const Route = createFileRoute("/api/public/shopify/webhooks/products-create")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const { verifyAndParseShopifyWebhook, reserveWebhookId, markWebhookProcessed } =
          await import("@/lib/shopify-webhook.server");
        const result = await verifyAndParseShopifyWebhook(request, "products/create");
        ...
        try {
          ...
          console.error("[products-create] handler error", e);
